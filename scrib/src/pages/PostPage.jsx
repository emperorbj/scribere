...
//   const {id} = useParams();
//   useEffect(() => {
//     fetch(`https://scriberebackend.vercel.app/post/${id}`)
//       .then(response => {
...
export default function PostPage() {
  ...
  if (loading) { // [CHANGED]
    return (
      <div className="container -mt-[40px] mx-auto px-4 py-8 max-w-2xl xl:max-w-4xl">
        <Skeleton height={40} width={"60%"} className="mb-4" /> {/* Skeleton for title */}
        <div className="flex items-center justify-between text-gray-500 text-sm mb-8">
          <Skeleton width={100} /> {/* Skeleton for author and date */}
        </div>
        <Skeleton height={400} className="mb-8" /> {/* Skeleton for the image */}
        <Skeleton count={5} className="mb-8" /> {/* Skeleton for the content */}
      </div>
    );
  }
  ...
  return (
    <div className="container -mt-[40px] mx-auto px-4 py-8 max-w-2xl xl:max-w-4xl">
      ...
      <div className="mb-8">
        <img src={postInfo?.cover} alt={postInfo?.title} className="rounded-lg w-full h-[400px] object-cover shadow-md" />
      </div>
      <div className="prose prose-lg text-gray-700 mb-8">
        <div dangerouslySetInnerHTML={{ __html: postInfo.content }} />
      </div>
      <div className="flex justify-end">
        <Link to="/" className="text-indigo-600 hover:text-indigo-800">← Back to posts</Link>
      </div>
    </div>
  );
}
